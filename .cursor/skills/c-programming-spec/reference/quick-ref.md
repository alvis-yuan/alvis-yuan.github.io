...
| 整函数 | 函数体 `{` 后、首条语句前 |
| 单个 `{ … }` 块 | 块 `{` 后、块内首条语句前 |
| `for` 计数器 | `for (int i = …)` 初始化子句 |

**禁：** 定义与可执行语句交错；C89 式顶堆声明后远处赋值。
...
| 元素 | 规则 |
| --- | --- |
| 存储类 + 返回类型 + 函数名 | **同一行**（`.h` 声明与 `.c` 定义一致） |
| 参数列表 | 过长时仅参数换行缩进 |
| 左花括号 `{` | 函数体另起一行；控制流在行尾 |

**禁：** 返回类型与函数名拆到不同行（含 `static inline`）。
...
| --- | --- |
| 源码注释、`//`、`/* */`、Doxygen | **中文** |
| `LogError`、`LogInfo`、`LOG_*` format 字符串 | **英文** |

**禁：** 日志中文；用英文行内注释替代中文设计说明。
...
16. 局部变量定义与可执行语句交错混放
17. 返回类型与函数名分行（违反 K&R 函数格式）
18. `LogError`/`LogInfo` 等日志字符串使用中文
